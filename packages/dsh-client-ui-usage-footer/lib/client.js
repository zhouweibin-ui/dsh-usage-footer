window.__ModuleLoader__.load({
	...
	/** Peak/off-peak switch instant (2026-08-17 00:00 Beijing), epoch ms. */
	const PEAK_EFFECTIVE_AT = Date.UTC(2026, 7, 17, 0, 0) - 8 * 3600e3;
	/**
	 * Built-in active card: flat before the switch, peak afterwards; returns
	 * the bracket-shaped table (flat normalizes to offPeak === peak).
	 * @param modelId - the session's model id (may be null).
	 * @returns the price table.
	 */
	function builtinTableOf(modelId) {
		const flat = Date.now() < PEAK_EFFECTIVE_AT;
		const table = flat ? PRICING_FLAT : PRICING;
		const price = table[modelId] ?? table["deepseek-v4-pro"];
		return flat ? { offPeak: price, peak: price } : price;
	}
	/**
	...
	const PEAK_HOURS = new Set([9, 10, 11, 14, 15, 16, 17]);
		...
		function costEstimate(usage, modelId, served, servedTable) {
			const price = priceOf(modelId, served, servedTable);
		...
		function costOfBuckets(buckets, modelId, served, servedTable) {
			const price = priceOf(modelId, served, servedTable);
		...
		function todayCostOf(record, served, servedTable) {
			let offPeak = 0;
			let peak = 0;
			for (const [modelId, buckets] of Object.entries(todayModelsOf(record))) {
				const cost = costOfBuckets(buckets, modelId, served, servedTable);
		...
		function UsageWidget(props) {
			...
			const servedTable = status !== null && (status.pricingTable === "flat" || status.pricingTable === "peak") ? status.pricingTable : null;
			const servedPricing = servedTable !== null && typeof status.pricing === "object" ? status.pricing : null;
			const flatCard = (servedTable === "flat") || (servedTable === null && Date.now() < PEAK_EFFECTIVE_AT);
			const total = usage !== undefined ? totalTokens(usage) : 0;
			const estimate = usage !== undefined && total > 0 ? costEstimate(usage, modelId, servedPricing, servedTable) : null;
			const todayTotal = todayTokensOf(today);
			const todayCost = todayTotal > 0 ? todayCostOf(today, servedPricing, servedTable) : { offPeak: 0, peak: 0 };
			...
			const panel = jsx("div", {
				...
				children: jsxs(Fragment, { children: [
					...
					jsxs(Fragment, { children: [
						...
						jsx("div", { className: "uW_costNote", children: t("panel.costNote", { model: modelId ?? "deepseek-v4-pro", source: (servedPricing !== null ? t("panel.costSourceDocs") : t("panel.costSourceBuiltin")) + " · " + (flatCard ? t("panel.costTableFlat") : t("panel.costTablePeak")) }) })
					] }),
		...
		const zh = {
			...
			"panel.costNote": "按 {model} 价目估算（{source}）",
			"panel.costSourceDocs": "价格来自官方定价页，自动同步",
			"panel.costSourceBuiltin": "内置价目表（自动同步不可用）",
			"panel.costTableFlat": "当前平价表（峰谷价 8/17 起切换）",
			"panel.costTablePeak": "峰谷价目表",
			"panel.month": "本月账户用量",
		...
		const en = {
			...
			"panel.costNote": "Estimated from {model} pricing ({source})",
			...
			"panel.costTableFlat": "flat rate card (peak/off-peak from 08-17)",
			"panel.costTablePeak": "peak/off-peak card",
